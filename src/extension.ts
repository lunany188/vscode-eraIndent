...
export class EraIndentProvider implements vscode.DocumentFormattingEditProvider, vscode.DocumentRangeFormattingEditProvider, vscode.OnTypeFormattingEditProvider {
    ...
    public provideDocumentFormattingEdits(document: vscode.TextDocument, options: vscode.FormattingOptions, token: vscode.CancellationToken): vscode.ProviderResult<vscode.TextEdit[]> {
        ...
        return this.innnerFormat(document, document.lineAt(document.lineCount - 1).range.end.line, indenter);
    }
    ...
    private innnerFormat(doc: vscode.TextDocument, endline: number, indenter: indentor.EraIndenter): vscode.TextEdit[] {
        ...
        for (let lineNo = 0; lineNo < endline; lineNo++) {
            ...
            if (indentor.isEraIndenterError(result)) {
                const newDiagnostic = new vscode.Diagnostic(doc.lineAt(result.lineNumber).range, result.message);
                nextDiags.push(newDiagnostic);
                continue;
            }
            result.forEach(ns => ret.push(vscode.TextEdit.replace(doc.lineAt(ns.lineNumber).range, ns.text)));
        }
